...
export function PhoneSimulator({ color, pattern, status, logoUrl, className, intensity = 1 }: PhoneSimulatorProps) {
  ...
  return (
    <div
      ...
      <div className='relative w-full h-full flex flex-col bg-bg text-text'>
        ...
        <div
          className={cn('flex-1 flex flex-col items-center justify-center relative transition-colors duration-300', getAnimationClass())}
          style={{
            backgroundColor: color === '#000000' ? '#1a1a1a' : color,
            boxShadow: color !== '#000000' && color !== '#FFFFFF' ? `inset 0 0 60px ${color}` : 'none',
          }}>
          {/* Logo/Mark Overlay */}
        ...
        <div className='h-32 bg-surface2/90 backdrop-blur-md border-t border-border/50 p-4 flex flex-col items-center justify-between z-10'>
          <div className='flex flex-col items-center gap-2'>
            ...
            />
            <p className='text-xs font-mono tracking-wider text-muted'>{statusText[status]}</p>
          </div>
