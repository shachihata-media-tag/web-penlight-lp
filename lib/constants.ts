export const PENLIGHT_COLORS = [
  { name: "Red", value: "#FF0000" },
  { name: "Blue", value: "#0000FF" },
  { name: "White", value: "#FFFFFF" },
  { name: "Orange", value: "#FFA500" },
  { name: "Green", value: "#008000" },
  { name: "Purple", value: "#800080" },
  { name: "Pink", value: "#FFC0CB" },
  { name: "Yellow", value: "#FFFF00" },
  { name: "Light Green", value: "#90EE90" },
  { name: "Light Blue", value: "#ADD8E6" },
  { name: "Light Pink", value: "#FFB6C1" },
  { name: "Violet", value: "#EE82EE" },
  { name: "Lime", value: "#00FF00" },
  { name: "Turquoise", value: "#40E0D0" },
];

export const DEMO_PRESETS = [
  {
    id: "intro",
    label: "オープニング",
    description: "高揚感を煽る点滅演出",
    sequence: ["Red", "Off", "Blue", "Off", "White"], // Simplified sequence representation
  },
  {
    id: "chorus",
    label: "サビ",
    description: "会場の一体感を生む点灯",
    sequence: ["Pink", "Light Pink", "Pink", "Light Pink"],
  },
  {
    id: "finale",
    label: "エンディング",
    description: "感動的なグラデーション",
    sequence: ["White", "Yellow", "Orange", "Red"],
  },
];

export const FAQS = [
  { q: "専用アプリのインストールは必要ですか？", a: "不要です。標準ブラウザ（Safari/Chrome）で動作するため、QRコードを読み込むだけですぐに参加できます。" },
  { q: "オフライン（電波がない環境）でも使えますか？", a: "初回読み込み（ページ表示）には通信が必要ですが、一度読み込めば、演出信号の受信自体はマイクを使用するため通信環境に依存せず動作します。" },
  { q: "バッテリーの消費は激しいですか？", a: "画面を常時点灯させ、マイク機能を使用するため通常のブラウジングよりは消費しますが、ゲームアプリ等と比較すると軽微です。" },
  { q: "アーカイブ配信でも連動しますか？", a: "はい。動画の音声に信号が含まれていれば、YouTube等のアーカイブ配信でも同様に連動します。" },
  { q: "アンドロイド端末でも使えますか？", a: "はい。Android 10以降のChromeブラウザで動作確認を行っております。" },
  { q: "同時に何人まで参加できますか？", a: "音響連動方式のため、サーバー負荷は小さく、理論上人数制限はありません。数万人規模のドーム実績もございます。" },
  { q: "導入にかかる期間は？", a: "最短5営業日での納品が可能です（素材受領後）。カスタマイズ内容により異なります。" },
  { q: "信号音は耳に聞こえますか？", a: "特殊な高周波帯域を使用しているため、通常の音楽や環境音に紛れてほとんど聞こえません。" },
  { q: "ロゴの差し替え費用は？", a: "基本パッケージに含まれます。イベントごとに異なるロゴを設定可能です。" },
  { q: "自分のスマホが対応しているか確認したい", a: "デモページ（このLPのシミュレーター）が動作すれば、本番環境でも問題なく動作します。" },
  { q: "スマホ以外の推し活グッズ（物理ペンライトやアクスタ）も連動できますか？", a: "はい、可能です。音響通信（Another Track®）の信号を受信できるモジュールを組み込んだ専用のペンライトや、光るアクスタ（アクリルスタンド）など様々な推し活グッズを展開し、イベントの音と同期させることができます。" },
];

export const SITE_METADATA = {
  title: "音響連動ソリューション｜スマホペンライト・ライブ演出 | Shachihata Inc.",
  description:
    "シヤチハタ株式会社の「音響連動ソリューション」は、専用アプリ不要でスマホがペンライトに変わる次世代の推し活・ライブ演出システムです。音響透かし技術（Another Track®）で会場と配信を同期。ドーム5万人規模の実績あり。QRコード読み取りだけで即参加でき、物理ペンライトやアクスタ（アクリルスタンド）などの推し活グッズ展開も可能。",
  siteName: "Shachihata 音響連動ソリューション",
  keywords: [
    // Primary
    "Webペンライト",
    "スマホペンライト",
    "音響連動ペンライト",
    "スマホ連動ペンライト",
    // Brand
    "シヤチハタ",
    "シャチハタ",
    "Shachihata",
    "Another Track",
    // Use Cases
    "ライブ演出",
    "コンサート演出",
    "イベント演出",
    "スポーツイベント演出",
    "音楽ライブ ペンライト",
    "配信連動",
    "オンラインライブ 一体感",
    // Technology
    "音響透かし",
    "音響通信技術",
    "QRコード ペンライト",
    "アプリ不要 ペンライト",
    "ブラウザ ペンライト",
    // Products
    "観客参加型演出",
    "音響連動アクリルスタンド",
    "アクスタ",
    "推し活グッズ",
    "推し活",
    "立体シール スマホ",
    "電子ペンライト",
    // Scale
    "ドーム ライブ演出",
    "大規模イベント 演出",
    "5万人同時接続",
    // Long-tail
    "ペンライト アプリ不要",
    "スマホ フラッシュ ペンライト",
    "配信 ペンライト 連動",
    "イベント 一体感 演出",
  ],
  url: "https://www.shachihata-x-light.com",
  ogImage: "https://www.shachihata-x-light.com/og-image.png",
};

export const GOODS_METADATA = {
  title: "ペンライトモード｜スマホが光る体験デモ",
  description:
    "QRコードからアクセスするだけで、あなたのスマホがペンライトに。14色のカラー切替、トーチ連動、好きなグッズ画像の表示が可能な、アプリ不要のWebペンライト体験ページです。",
  keywords: [
    "スマホペンライト",
    "Webペンライト体験",
    "ペンライトアプリ不要",
    "スマホ ライブ ペンライト",
    "QRコード ペンライト",
    "推し活",
    "アクスタ",
  ],
};
